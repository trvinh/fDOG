...
def checkOptConflict(lib, conda):
    if lib:
        if (conda):
            sys.exit(
                '*** ERROR: --lib and --conda cannot be used at the same time!')


def main():
    version = '0.0.3'
    parser = argparse.ArgumentParser(
        description='You are running fdog.setup version ' +
        str(version) +
        '.')
    ...
    required.add_argument(
        '-o',
        '--outPath',
        help='Output path for fdog data',
        action='store',
        default='',
        required=True)
    optional.add_argument(
        '--conda',
        help='Setup fdog within a conda env',
        action='store_true',
        default=False)
    optional.add_argument(
        '--lib',
        help='Install fdog libraries only',
        action='store_true',
        default=False)
    optional.add_argument(
        '--getSourcepath',
        help='Get path to installed fdog',
        action='store_true',
        default=False)
    optional.add_argument(
        '--getDatapath',
        help='Get fdog default data path',
        action='store_true',
        default=False)
